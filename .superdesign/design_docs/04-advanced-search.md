...
interface SearchCriteria {
  ...
  tags?: string[]; // 标签过滤
  priorities?: string[]; // 优先级筛选（支持多选：['high', 'medium']）
...
- 点击未选中的按钮 → 添加该优先级到过滤条件
- 点击已选中的按钮 → 从过滤条件中移除该优先级
- 可同时选择多个优先级（如 high + medium）
- 所有按钮都未选中时，显示全部优先级的规则
- 选择优先级后，需点击"搜索"按钮执行搜索
...
- 只看高优先级：点击 High
- 看中高优先级：点击 High，再点击 Medium
- 取消高优先级：再次点击 High（只剩 Medium）
- 显示全部：点击 Medium 取消所有选择
