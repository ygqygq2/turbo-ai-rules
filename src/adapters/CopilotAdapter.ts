...
export class CopilotAdapter extends BaseAdapter {
  ...
  async generate(rules: ParsedRule[]): Promise<GeneratedConfig> {
    ...
    // 添加目录
    content += this.generateTaggedTableOfContents(groupedByTag);
  ...
  /**
   * 生成目录（按标签分组）
   * @param groupedByTag 按标签分组的规则
   * @returns 目录内容
   */
  private generateTaggedTableOfContents(groupedByTag: Map<string, ParsedRule[]>): string {
