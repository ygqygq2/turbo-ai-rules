...
describe('Cursor Workflow Tests', () => {
  ...
  const testSourceUrl = 'https://github.com/ygqygq2/ai-rules.git';
  ...
  it('Should complete full end-to-end workflow', async function () {
    ...
    // 步骤1: 添加规则源（模拟 UI 输入）
    mockShowInputBox(testSourceUrl);
    mockShowQuickPick({ label: 'None', description: 'Public repository' } as vscode.QuickPickItem);

    const config = vscode.workspace.getConfiguration('turbo-ai-rules', workspaceFolder.uri);
    const sourcesBefore = config.get<any[]>(CONFIG_KEYS.SOURCES, []);

    await vscode.commands.executeCommand('turbo-ai-rules.addSource');
    await sleep(TEST_DELAYS.MEDIUM);

    const sourcesAfter = config.get<any[]>(CONFIG_KEYS.SOURCES, []);
    assert.ok(sourcesAfter.length > sourcesBefore.length, 'Source should be added');

    const addedSource = sourcesAfter.find((s) => s.gitUrl === testSourceUrl);
