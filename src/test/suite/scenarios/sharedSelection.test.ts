...
describe('Shared Selection Integration Tests', () => {
  ...
  before(async function () {
    ...
    // 查找 scenarios-sharedSelection 工作区
    enabledWorkspace = folders.find(
      (f) =>
        f.name === 'Scenarios: Shared Selection' ||
        f.name.includes('sharedSelection') ||
        f.uri.fsPath.includes('sharedSelection'),
    )!;
    assert.ok(enabledWorkspace, 'Enabled workspace should be available');

    // 使用第一个工作区作为禁用工作区（用于对比测试）
    disabledWorkspace = folders.find((f) => f !== enabledWorkspace) || folders[0];
