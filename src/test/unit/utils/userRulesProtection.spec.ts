...
describe('userRulesProtection 单元测试', () => {
  ...
  describe('extractPrefix', () => {
    it('应该提取数字前缀', () => {
      ...
      expect(extractPrefix('850-user-rules.md')).toBe(850);
      expect(extractPrefix('999-custom.md')).toBe(999);
    });
  ...
  describe('isUserDefinedFile', () => {
    it('应该识别用户定义范围的文件(800-999)', () => {
      expect(isUserDefinedFile('800-user-start.md', enabledConfig)).toBe(true);
      expect(isUserDefinedFile('850-my-rules.md', enabledConfig)).toBe(true);
      expect(isUserDefinedFile('999-user-end.md', enabledConfig)).toBe(true);
    });

    it('应该拒绝自动生成范围的文件(0-799)', () => {
      expect(isUserDefinedFile('000-auto.md', enabledConfig)).toBe(false);
      expect(isUserDefinedFile('500-typescript.md', enabledConfig)).toBe(false);
      expect(isUserDefinedFile('799-auto-end.md', enabledConfig)).toBe(false);
    });
    ...
    it('应该在禁用保护时返回 false', () => {
      expect(isUserDefinedFile('850-user-rules.md', disabledConfig)).toBe(false);
    ...
    it('应该支持自定义范围', () => {
      ...
      expect(isUserDefinedFile('850-out-range.md', customRangeConfig)).toBe(false);
  ...
  describe('isAutoGeneratedFile', () => {
    it('应该识别自动生成范围的文件', () => {
      ...
      expect(isAutoGeneratedFile('799-end.md', enabledConfig)).toBe(true);
    });

    it('应该拒绝用户定义范围的文件', () => {
      expect(isAutoGeneratedFile('800-user.md', enabledConfig)).toBe(false);
      expect(isAutoGeneratedFile('999-user.md', enabledConfig)).toBe(false);
    });
    ...
    it('应该在禁用保护时返回 true', () => {
      expect(isAutoGeneratedFile('850-user.md', disabledConfig)).toBe(true);
  ...
  describe('filterAutoGeneratedFiles', () => {
    ...
    it('应该在禁用保护时返回所有文件', () => {
      const files = ['/path/001-auto.md', '/path/850-user.md', '/path/readme.md'];
  ...
  describe('mergeContent', () => {
    ...
    it('应该在没有用户内容时添加模板', () => {
      ...
      expect(merged).toContain('Your Custom Rules');
      expect(merged).toContain('Add your custom rules below');
      expect(merged).toContain('Team Conventions'); // 示例部分
    });

    it('应该包含时间戳', () => {
      const merged = mergeContent('auto', 'user', enabledConfig);

      expect(merged).toMatch(/Last Sync: \d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
    });
  ...
  describe('detectConflicts', () => {
    it('应该检测用户范围的文件冲突', () => {
      const existingFiles = ['/path/850-user.md', '/path/900-custom.md'];
      const newFiles = ['/path/850-user.md', '/path/001-auto.md'];

      const conflicts = detectConflicts(existingFiles, newFiles, enabledConfig);

      expect(conflicts).toHaveLength(1);
      expect(conflicts).toContain('850-user.md');
    });
    ...
    it('应该在禁用保护时返回空数组', () => {
      const existingFiles = ['/path/850-user.md'];
      const newFiles = ['/path/850-user.md'];
  ...
  describe('suggestAlternativeFilename', () => {
    it('应该建议自动范围内的文件名', () => {
      const result = suggestAlternativeFilename('850-rules.md', []);
    ...
    it('应该避免已存在的前缀', () => {
      const existingFiles = ['/path/799-exist.md', '/path/798-exist.md', '/path/797-exist.md'];

      const result = suggestAlternativeFilename('850-new.md', existingFiles);

      const prefix = extractPrefix(result);
      expect(prefix).toBe(796); // 第一个未被占用的
  ...
  describe('常量验证', () => {
    ...
    it('应该有正确的前缀范围', () => {
      ...
      expect(PREFIX_RANGES.AUTO_MAX).toBe(799);
      expect(PREFIX_RANGES.USER_MIN).toBe(800);
      expect(PREFIX_RANGES.USER_MAX).toBe(999);
    });
  });
  ...
  describe('集成场景', () => {
    it('应该支持完整的文件保护流程', () => {
      // 1. 检测文件是否为用户文件
      const filename = '850-my-rules.md';
      expect(isUserDefinedFile(filename, enabledConfig)).toBe(true);

      // 2. 过滤时排除用户文件
      const files = ['/path/001-auto.md', `/path/${filename}`];
