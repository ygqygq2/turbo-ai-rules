...
export class ExtensionMessenger {
    ...
    this.webview.onDidReceiveMessage(async (message: Message) => {
      ...
      if (requestId) {
        try {
          ...
          this.sendResponse(requestId, undefined, error.message || 'Handler error');
        }
      } else {
        // 无 requestId，说明是单向通知
        try {
   ...
   * @param payload {any} 响应数据
   * @param error {string} 错误信息
   */
  private sendResponse(requestId: string, payload?: any, error?: string): void {
  ...
  /**
   * @description 推送事件到 Webview（无需响应）
   * @return {void}
