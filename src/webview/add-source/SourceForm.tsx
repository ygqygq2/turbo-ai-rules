...
export const SourceForm: React.FC<SourceFormProps> = ({
  ...
  useEffect(() => {
    if (isEditMode) {
      vscodeApi.postMessage('loadSourceData', { sourceId });
    }
  }, [isEditMode, sourceId]);
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message.type === 'addSourceStatus' || message.type === 'updateSourceStatus') {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const messageType = isEditMode ? 'updateSource' : 'addSource';
    ...
    vscodeApi.postMessage(messageType, payload);
  };
