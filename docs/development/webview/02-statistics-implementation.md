...
- 标签改为 `<button>` 元素，添加 `onClick` 事件
- 发送 `searchByTag` 消息到 Extension
- Extension 打开搜索页面并预填标签条件
- 自动执行搜索显示结果

**技术要点**:

- 使用动态 import 避免循环依赖
- SearchWebviewProvider 添加 `showSearchWithCriteria` 方法
- 使用 setTimeout 确保面板准备就绪后再发送数据
- 支持多个标签同时搜索（数组格式）
