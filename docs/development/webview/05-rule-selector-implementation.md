...
> **对应设计文档**：`.superdesign/design_docs/05-rule-selector.md`  
> **UI 原型**：`.superdesign/design_iterations/05-rule-selector_2.html`  
> **状态**：已实现（树形结构 + Zustand 状态管理）
...
1. **多源切换**：当工作区有多个规则源时，通过下拉框切换源
2. **树形展示**：以 VSCode 资源管理器风格显示目录/文件层级结构
3. **选择规则**：通过复选框选择/取消选择文件或目录
4. **批量操作**：全选/清除/重置按钮
5. **搜索过滤**：按标题、路径、标签搜索
6. **持久化**：保存选择配置到工作区缓存，下次打开自动恢复
