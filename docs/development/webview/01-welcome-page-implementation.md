...
> **HTML 原型**: `.superdesign/design_iterations/welcome-page_v*.html`  
> **实施日期**: 2025-10-27  
> **状态**: ✅ 已完成
...
实现首次使用引导页面，帮助新用户快速上手 Turbo AI Rules 扩展。

### 核心功能

1. **快速开始流程**: 3 步引导（添加源 → 同步规则 → 生成配置）
2. **模板库**: 提供常用规则源模板
3. **文档入口**: 快速访问文档和帮助
4. **智能显示**: 根据用户状态自动判断是否显示
...
| 消息类型 | 用途 | 触发时机 |
|---------|------|---------|
| `addSource` | 添加规则源 | 点击"添加源"按钮 |
| `syncRules` | 同步规则 | 点击"同步规则"按钮 |
| `generateConfigs` | 生成配置 | 点击"生成配置"按钮 |
| `useTemplate` | 使用模板 | 点击模板卡片 |
| `viewDocs` | 查看文档 | 点击文档链接 |
| `dismiss` | 关闭页面 | 点击关闭按钮 |
