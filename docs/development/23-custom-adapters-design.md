...
扩展只同步和管理来自配置源的规则：
- **远程规则**: 来自 Git 仓库（通过 `sources` 配置），ID 范围 00000-79999
- **用户规则**: 来自 `ai-rules/` 目录（通过 `userRules.directory` 配置），建议 ID 范围 80000-99999

**ID 范围约定**:

- `00000-79999`: 远程规则（自动生成）
- `80000-99999`: 用户规则（手动创建，建议使用此范围）

**为什么建议 80000-99999？**

- 📋 **清晰区分**: 一眼就能区分哪些是远程规则，哪些是用户规则
- 🔢 **排序稳定**: 使用 ID 排序时，用户规则位置稳定且可预期
- 🎯 **避免冲突**: 与远程规则 ID 完全不重叠，避免冲突

**清理逻辑**:

生成配置时，扩展会：
1. 计算期望文件列表 = 选中的远程规则 + ai-rules/ 中的用户规则
2. 删除不在期望列表中的文件
3. 保留 `index.md`（索引文件）
...
2. 添加 `.md` 或 `.mdc` 规则文件，使用 80000-99999 ID
3. 执行 `Generate Config Files` 命令
4. 自定义规则会被自动包含在生成的配置中
...
```
ai-rules/
  ├── 80001-team-conventions.md   (用户规则，ID: 80001)
  ├── 85000-api-guidelines.md     (用户规则，ID: 85000)
  └── 90000-code-review.md        (用户规则，ID: 90000)

.someAdapter/rules/
  ├── 00001-remote-rule-1.md      (远程规则，ID: 00001)
  ├── 00002-remote-rule-2.md      (远程规则，ID: 00002)
  ├── 80001-team-conventions.md   (来自 ai-rules/)
  ├── 85000-api-guidelines.md     (来自 ai-rules/)
  └── 90000-code-review.md        (来自 ai-rules/)
```
...
- 适配器设计: `docs/development/21-adapter-design.md`
