...
│   │   └── *.spec.ts             # 服务测试
│   ├── providers/
│   │   └── RuleSelectorWebviewProvider.spec.ts  # 仅测试业务逻辑（规则选择）
│   ├── setup.ts                   # 测试环境设置
│   └── *.spec.ts                  # 其他单元测试
├── suite/                         # 集成测试
│   └── *.test.ts                  # VSCode 集成测试
...
> **注意**：
>
> - Webview Provider 的 UI 交互测试（WelcomeWebviewProvider、SearchWebviewProvider、RuleDetailsWebviewProvider）已移除
> - 原因：这些测试属于 UI 层面，依赖复杂的 mock 设置，维护成本高，应通过手动测试或集成测试验证
> - 保留的 RuleSelectorWebviewProvider 测试仅测试业务逻辑（规则选择算法），不测试 UI 交互
...
>
> - UI 测试依赖复杂的 mock 设置（postMessage、panel 状态等）
> - 维护成本高，经常因实现细节变化而失败
> - 应通过手动测试或端到端集成测试验证
> - 核心业务逻辑已在 Services/Commands 层充分测试
...
✅ **核心功能**: 同步、源管理、配置生成（100% 通过）
✅ **适配器**: 4 种主要适配器 + 自定义适配器
✅ **高级功能**: 多源、冲突解决、用户规则保护
✅ **Git 操作**: 克隆、拉取、分支、SubPath
✅ **文件系统**: 生成、保护、gitignore
✅ **配置管理**: 多级配置、合并策略
✅ **测试覆盖**: 36 个测试文件，357 个测试用例，100% 通过

⚠️ **需要手动测试**: 认证、UI 交互、网络错误、性能

**测试统计**（最后更新: 2025-11-25）:
...
- Duration: ~46s
- 测试场景:
  - ✅ User Rules Protection (3 tests)
  - ✅ Sync Rules (3 tests)
  - ✅ Search Rules (3 tests)
  - ✅ Remove Source (3 tests)
  - ✅ Pre-configured Sources (3 tests)
  - ✅ Multi-Source Integration (4 tests)
  - ✅ Generate Config Files (1 test)
  - ✅ Extension Activation (2 tests)
  - ✅ Context Menu Commands (3 tests)
  - ✅ Batch Operations (2 tests)
  - ✅ Add Source (3 tests)

通过这些测试场景，可以确保插件核心功能的稳定性和正确性。UI 层面的 Webview 交互通过手动测试和集成测试验证。
