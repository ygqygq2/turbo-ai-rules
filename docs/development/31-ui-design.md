...
- 文件树构建：使用 `MdcParser.parseDirectory()` 结果
- 过滤规则：自动排除 README.md 等配置文件
- 路径处理：统一使用相对路径存储和比对
- 与左侧树视图共享相同的数据源和选择状态
...
- `addSource` - 添加规则源
- `syncRules` - 同步规则（快捷同步规则到启用的规则类型适配器）
- `syncToAdapters` - 规则同步页的同步操作（新增）
- `generateRules` - 生成配置文件
