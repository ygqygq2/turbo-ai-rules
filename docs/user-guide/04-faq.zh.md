...
**方法 1: 目录模式 - 使用 ai-rules/ 目录**

**核心机制**: 扩展只管理来自配置源的规则（Git 仓库 + ai-rules/ 目录），不在期望列表中的文件会被删除。

**为什么使用 ai-rules/ 目录？**

- 🛡️ **统一管理**: 所有用户自定义规则集中在一个目录，易于管理
- 📋 **版本控制**: 可以纳入 Git 管理，团队共享
- 🔒 **自动保护**: 生成配置时会自动包含 ai-rules/ 中的规则
- ⚙️ **灵活配置**: 支持适配器级别的启用控制（enableUserRules）

**操作步骤**：

```bash
# 1. 在工作区根目录创建 ai-rules/ 目录
mkdir ai-rules

# 2. 创建自定义规则文件（建议使用 80000-99999 ID）
cat > ai-rules/80001-team-specific.md << 'EOF'
---
id: 80001
title: 团队特定规则
priority: high
...
- **00000-79999**: 远程规则（自动生成）
- **80000-99999**: 用户规则（建议使用此范围）

**为什么建议 80000-99999？**

- 📋 **清晰区分**: 一眼就能区分远程规则和用户规则
- 🔢 **排序稳定**: 使用 ID 排序时，用户规则位置稳定且可预期
- 🎯 **避免冲突**: 与远程规则 ID 完全不重叠
...
```json
{
  "turbo-ai-rules.userRules": {
    "directory": "ai-rules"  // 用户规则目录（默认）
  },
  "turbo-ai-rules.adapters.cursor": {
    "enabled": true,
    "enableUserRules": true  // 启用用户规则（默认: true）
  }
}
# 1. 进入 rules 目录
cd .cursor/rules
...
- ⚠️ 扩展会尝试自动检测当前活跃的工作空间文件夹
- ⚠️ 用户规则（`ai-rules/` 目录）会写入到检测到的当前工作空间
- ⚠️ 可能在某些场景下无法准确识别工作空间上下文
- ⚠️ 多工作空间环境未经过完整测试

**已知改进**:

- ✅ v2.0.3: 修复了用户规则在多工作区环境下被清理的问题
- ✅ CustomAdapter 和 FileGenerator 现在使用工作空间上下文管理器

**示例场景**:

```
workspace.code-workspace
├─ project-a/  ← ⚠️ 扩展会尝试检测当前活跃的工作空间
├─ project-b/  ← ⚠️ 可能需要手动切换
└─ project-c/  ← ⚠️ 可能需要手动切换
```

**推荐做法**:

1. **单工作空间优先**: 如果需要为多个项目管理规则，建议在不同的 VS Code 窗口中分别打开它们
2. **小心验证**: 在多工作空间中使用时，请仔细检查生成的文件位置是否正确
3. **备份配置**: 在多工作空间环境中首次使用前，建议备份现有配置

**配置建议** (如果确实需要使用多工作空间):

1. **工作区级配置**: 共享规则源可以放在 `.code-workspace` 中

```json
{
  "settings": {
    "turbo-ai-rules.sources": [
      { "id": "company-rules", "gitUrl": "..." }
    ]
  }
}
```

2. **文件夹级配置**: 每个项目特定的配置放在各自的 `.vscode/settings.json` 中

**潜在问题**:

- 在 Webview 和编辑器之间切换时可能丢失工作空间上下文
- 规则选择状态管理在多工作空间场景下可能不够精确
- 某些命令可能无法准确判断应该操作哪个工作空间

**未来计划**: 可能会根据用户反馈和测试结果在未来版本中逐步完善多工作空间支持。如果你在多工作空间环境中遇到问题，欢迎提 [Issue](https://github.com/ygqygq2/turbo-ai-rules/issues)。
